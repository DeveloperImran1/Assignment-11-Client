...
import { homeGet, removeHome } from "../CustomHooks/LocasStorage";
...
const Bookmarks = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const allEstates = await homeGet(); // Assuming getEstate returns a promise
    ...
    if (estates.length === 0) {
        return <section className="flex items-center h-[440px] rounded-[16px] sm:p-16 bg-base-200">
            ...
            <div className="container flex flex-col items-center justify-center px-5 mx-auto my-8 space-y-8 text-center sm:max-w-md">
                ...
                <p className="text-3xl dark:text-gray-600 ">Estate not Selected!</p>
    ...
    return (
        <div className="grid  justify-center grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            {
                estates.map(estate => <BookmarkList key={estate.id} estate={estate} handleRemoveEstate={handleRemoveEstate} ></BookmarkList> )
            }
        </div>
    );
};
